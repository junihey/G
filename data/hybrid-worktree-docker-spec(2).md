...
| **Verwaltung der physischen Umgebung** | Worktree anlegen/löschen, optional Container mounten/stoppen | **Sandcastle** |
...
function getProvider(mode: AgentMode, path: string, dockerImage?: string) {
...
export async function runAgent(store: IssueStore, spec: TaskSpec) {
  ...
  try {
    ...
    await wt.run({ provider, prompt: `Fix issue ${spec.issueId}`, skills: ["./skills/tdd-loop.md"] });
...
## 18. Build-Phasen (Roadmap)

| Phase | Inhalt | Ergebnis |
|---|---|---|
| 0 – Setup | Sandcastle, `vault/issues/`-Ordner, Frontmatter-Schema | Grundgerüst |
| 1 – Vorstufe-Skills | `grill-me`, `write-a-prd`, `prd-to-issues` als Prompts | Von Idee zu Issue-Notes ohne manuelles Schreiben |
| 2 – Obsidian Issue-Store | `ObsidianIssueStore` (Chokidar + gray-matter), `onChange`-Dispatch | Statuswechsel im Frontmatter triggert Agent |
| 3 – Host-MVP | `run-agent.ts` nur `noSandbox()`, ein Agent | Einzelner Fix-Loop funktioniert |
| 4 – Docker-Provider | `dockerProvider()`, `chooseMode()` | Modus automatisch pro Issue |
| 5 – Routing Ralph/Merge-Queue | `slice`-Auswertung, Ralph-Loop, Prioritäts-Sortierung | Horizontale und vertikale Issues im passenden Modus |
| 6 – Merge-Queue | Neutraler Integrations-Worktree, FIFO, Human-in-the-Loop | Sichere Integration bei Parallelität |
| 7 – Cleanup-Automatisierung | Scan für verwaiste Worktrees/Branches/Container | Kein manuelles Aufräumen |
| 8 – GitHub-Fallback (optional) | `GitHubIssueStore`, Label-Taxonomie | Austauschbares Backend nur bei Team-Bedarf |
| 9 – Herdr-Sichtbarkeit (optional) | `herdr integration install claude`, Adapter aus 15.4, Notification-Hook | Terminal-Übersicht über alle laufenden Agenten, Status ohne eigenen Reporting-Code |
| 10 – Review-Rolle (optional, später) | Zusätzliches LLM-Review vor Merge (stärkeres Modell) | Fängt Probleme jenseits grüner Tests |
