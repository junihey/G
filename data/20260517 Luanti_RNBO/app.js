async function setup() {
    ...
    try {
        ...
        // --- START: Verbindung zu Luanti ---

        // 1. Verbinde dich mit der lokalen Node.js Bridge
        const ws = new WebSocket("ws://127.0.0.1:8080");

        // 2. Was passiert, wenn eine Nachricht kommt?
        ws.onmessage = (event) => {
            // Wandle den Text wieder in ein Daten-Objekt um
            const oscMsg = JSON.parse(event.data);
            
            // Prüfe, ob die Nachricht der Luanti-Befehl ist
            if (oscMsg.address === "/luanti/dig") {
                
                // Sende eine "1" (als Trigger) an den RNBO-Eingang namens "trig"
                const messageEvent = new RNBO.MessageEvent(RNBO.TimeNow, "trig", [1]);
                device.scheduleEvent(messageEvent);
                
                console.log("Block abgebaut! Sound getriggert.");
            }
        };

        ws.onopen = () => console.log("Websocket mit Bridge verbunden!");
        // --- ENDE: Verbindung zu Luanti ---
    } catch (err) {
        if (typeof guardrails === "function") {
