...
**In `zaehlen\` liegt nicht nur, was durch die Pipeline kam.** Seit dem 2026-09-08 steht dort `vps\` — die Skripte und compose-Dateien, die den IONOS-Server erzeugen. Der Ordner lag vorher unter `specs\vps-neuaufbau\` und war dort falsch: Ein Spec trägt `exploration.md`, `scope.md` und `raw\`, dieser Ordner trägt `bootstrap.sh` und `restore.sh`. **Die Trennlinie ist nicht die Herkunft, sondern was eine Datei tut.** Wer baut, geht nach `zaehlen\`, auch ohne je einen Spec gesehen zu haben.

Was dabei im Vault blieb, ist das Rohmaterial: `specs\vps-neuaufbau\raw\`. Denken bleibt, Bauen zieht um — der Satz weiter oben, dass zwischen den Wurzeln keine Datei wandert, meint keine Fehlablage, die man nie korrigieren dürfte.
